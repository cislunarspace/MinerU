...
from mineru.cli.llm_translate.edit_distance import MinDistance
...
class EasyHeaderCorrector:
    ...
    def do_correct(self, md_path, correct_md_path, encoding='utf-8'):
        ...
        correct_txt = self.llm_agent.ask_llm(prompt, header_txts)
        LOGGER.info('correct_txt: %s', correct_txt)
        if not correct_txt:
            LOGGER.error('llm response is empty, correct header failed')
            return False

        correct_headers = correct_txt.split('\n')
