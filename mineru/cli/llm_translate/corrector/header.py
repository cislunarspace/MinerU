...
from mineru.cli.llm_translate.llm_agent import llm_agent_factory
from mineru.cli.llm_translate.edit_distance import MinDistance, EditType
from mineru.cli.llm_translate.split_utils import split_md
...
class HeaderCorrector:
    def __init__(self):
        ...
        self.allow_miss_num = corrector_conf['allow_miss_num']
        self.allow_miss_num_add = 0
        self.allow_miss_depth = corrector_conf['allow_miss_depth']
    ...
    def remove_overlap_his(self, history_headers, correct_headers, header_section):
        if not history_headers or not correct_headers:
            return correct_headers
        ...
        for index, header in enumerate(correct_headers):
            ...
            if index >= len(history_headers):
                return correct_headers[index:]
